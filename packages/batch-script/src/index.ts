import { BatchWriteItemCommand, BatchWriteItemCommandInput, DeleteItemCommand, DeleteItemCommandInput, DynamoDBClient, PutItemCommand, ScanCommand, ScanCommandInput, UpdateItemCommand, UpdateItemCommandInput } from "@aws-sdk/client-dynamodb";
...
async function insertItems(client: DynamoDBClient) {
    ...
    for (let i = 0; i < TEST_SIZE; i++) {
        ...
        if (input.RequestItems?.["DynamoStack-TestTable5769773A-WBGE6PUF9M82"].length == DYNAMO_DB_MATCH_BATCH_SIZE) {
            const command = new BatchWriteItemCommand(input);
            await profiler.profileAsync('batch insert', async () => await client.send(command));
...
async function updateItems(client: DynamoDBClient) {
    ...
    do {
        ...
        const scanOutput = await profiler.profileAsync('scan', async () => await client.send(scanCommand));
        ...
        for (const item of scanOutput.Items) {
            ...
            if (input.RequestItems?.["DynamoStack-TestTable5769773A-WBGE6PUF9M82"].length == DYNAMO_DB_MATCH_BATCH_SIZE) {
                const command = new BatchWriteItemCommand(input);
                await profiler.profileAsync('batch update', async () => await client.send(command));
                input.RequestItems["DynamoStack-TestTable5769773A-WBGE6PUF9M82"] = [];
            }
        }
...
async function deleteItems(client: DynamoDBClient) {
    ...
    do {
        ...
        for (const item of scanOutput.Items) {
            ...
            if (input.RequestItems?.["DynamoStack-TestTable5769773A-WBGE6PUF9M82"].length == DYNAMO_DB_MATCH_BATCH_SIZE) {
                const command = new BatchWriteItemCommand(input);
                await profiler.profileAsync('batch delete', async () => await client.send(command));
                input.RequestItems["DynamoStack-TestTable5769773A-WBGE6PUF9M82"] = [];
            }
        }
...
async function main() {
    ...
    profiler.report((time) => { return { "time per item": Math.round((time/TEST_SIZE)*100)/100 }});
    console.log("Done!");
}
main();