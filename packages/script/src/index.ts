...
async function insertItems(client: DynamoDBClient) {
    ...
    for (let i = 0; i < TEST_SIZE; i++) {
        ...
        await profiler.profileAsync('insert', async () => await client.send(command));
    }
}
...
async function updateItems(client: DynamoDBClient) {
    ...
    do {
        ...
        for (const item of scanOutput.Items) {
            ...
            await profiler.profileAsync('update', async () => await client.send(updateCommand));
        }
...
async function deleteItems(client: DynamoDBClient) {
    ...
    do {
        ...
        for (const item of scanOutput.Items) {
            ...
            await profiler.profileAsync('delete', async () => await client.send(deleteCommand));
        }
